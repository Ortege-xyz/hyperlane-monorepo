...
impl OwnableUpgradeable {
    pub fn init(env: Env, owner: Address, initializable: Initializable) {
...
impl Contract {
    ...
    pub fn increment(env: Env, caller: Address) -> u32 {
        OwnableUpgradeable::only_owner(env.clone(), caller);
    ...
    // Ownable functions
    pub fn owner(env: Env) -> Address {
        return OwnableUpgradeable::owner(env);
    }

    pub fn renounce_ownership(env: Env, caller: Address) {
        OwnableUpgradeable::renounce_ownership(env, caller);
    }

    pub fn transfer_owner_ship(env: Env, caller: Address, new_owner: Address) {
        OwnableUpgradeable::transfer_owner_ship(env, caller, new_owner);
    }
}
